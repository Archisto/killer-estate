    ...
    public class GroundCollider : MonoBehaviour
    {
        ...
        private void UpdateOffGround()
        {
            ...
            if (!_isRising)
            {
                ...
                if (AbleToJump)
                {
                    _jumpDisableElapsedTime += GameManager.Instance.DeltaTime;
                }
            }
        }
        ...
        private void Fall()
        {
            ...
            float fallAmount = (1f + 2 * DistanceFallen) * GameManager.Instance.DeltaTime;
                //(World.Instance.gravity + 2 * DistanceFallen) * GameManager.Instance.DeltaTime;
        ...
        {
            float riseAmount = speed * GameManager.Instance.DeltaTime;
        ...
        private bool SmoothMove()
        {
            ...
            if (groundHeightDiff < 0.5f * _objectSize.y)
            {
                    ...
                    groundHeightDiff > 0.0001f * _objectSize.y))
                {
                    ...
                    //newPosition =
                    //    transform.position +
                    //    new Vector3(direction.x, groundHeightDiff, direction.y).normalized * _speed * GameManager.Instance.DeltaTime;
